...
Schedule_edf(void)
{
	...
	if (nexttsk != TSKID_NULL ){
		...
		// 現在ティック < キューのデッドライン && ( 現在ティック < 現在タスクのデッドライン && 現在タスクのデッドライン < キューのデッドライン ) もしくは
		// 現在ティック < キューのデッドライン && ( 現在ティック < 現在タスクのデッドライン < キューのデッドライン )
		// 現在ティック > キューのデッドライン && ( 現在ティック < 現在タスクのデッドライン || 現在タスクのデッドライン < キューのデッドライン )
		// キューのデッドライン < 現在ティック < 現在タスクのデッドライン || 現在タスクのデッドライン < キューのデッドライン < 現在ティック
		// 現在の最高優先度をプリエンプとさせる，タスクキューの先頭タスクではなくて．
		if((curval < dl && (curval < tcb_deadline[schedtsk] && tcb_deadline[schedtsk] < dl )) ||
